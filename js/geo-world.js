...
Promise.all([
    ...
    const borderLayer = svg.selectAll(".border")
        ...
        .attr("title", d => {
            if (d.properties.SOVEREIGNT === "China" || d.properties.SOVEREIGNT === "Taiwan") {
                return `<strong data-localize="${d.properties.SOVEREIGNT}">${d.properties.SOVEREIGNT}</strong><br><a class="link-light" href="geo-china">Jump to China Map</a</br>`;
            }
            return `<strong data-localize="${d.properties.SOVEREIGNT}">${d.properties.SOVEREIGNT}</strong><br><span data-localize="Receive">Received:</span> ${receivedCount[d.properties.SOVEREIGNT] || 0}<a class="text-decoration-none link-light" href="received?countries=${d.properties.SOVEREIGNT}" target="_blank">🔗</a> <span data-localize="avg">avg</span> ${receivedDeliveryDays[d.properties.SOVEREIGNT] || "-"} <span data-localize="day(s)">day(s)</span></br><span data-localize="Send">Sent:</span> ${sentCount[d.properties.SOVEREIGNT] || 0}<a class="text-decoration-none link-light" href="sent?countries=${d.properties.SOVEREIGNT}" target="_blank">🔗</a> <span data-localize="avg">avg</span> ${sentDeliveryDays[d.properties.SOVEREIGNT] || "-"} <span data-localize="day(s)">day(s)</span>`;
        });
